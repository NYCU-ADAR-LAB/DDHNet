...
class CreateEmbedding(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, base_dim, interaction_op):
        ...
        # elif self.interaction_op=='gate':
            # self.gate = nn.Linear(embedding_dim, 1, bias=False)
    ...
    def forward(self, input):
        ...
        # elif self.interaction_op=='gate':
            ...
            # gating = self.act(self.gate(emb_vec))
            # #return torch.mul(emb_vec, gating)
...
class DLRM(torch.nn.Module):
    ...
    def __init__(self,field_size, feature_sizes, embedding_size = 64, is_shallow_dropout = True, dropout_shallow = [0.0,0.0],
                 ...
                 threshold_type = 'feature_dim', threshold_init = -150, retrain = False, retrain_mlp_param = None, re_init = False):
        ...
        self.candidate_p = [int((1072129+150832) * factor) for factor in [0.50, 0.375, 0.25]]
        print('candidate_p: ', self.candidate_p)
        self.alias='DLRM_BaseDim{}_bsz{}_lr_{}_optim_{}_thresholdType{}_thres_init{}'.format(self.embedding_size, self.batch_size, self.learning_rate, self.optimizer_type, threshold_type, threshold_init)
        print(self.alias)
        self.opt = {
            ...
            'retrain_mlp_param': retrain_mlp_param,
            're_init': re_init,
            'mlp_save_path': './tmp/mlp/{alias}/{num_parameter}/{layer}'.format(
                                    alias=self.alias,
                                    num_parameter='{num_parameter}',
                                    layer='{layer}')
        }
        self._writer = SummaryWriter(log_dir='./tmp/runs/{}'.format(self.alias))
        ...
        self.net_bottom = nn.ModuleList()
        self.net_bottom.append(PEPMLP(self.opt, 'bot0', self.num, self.bottom_mlp[0]))
        for i,o in enumerate(self.bottom_mlp[1:], 1):
            self.net_bottom.append(PEPMLP(self.opt, 'bot'+str(i+1), self.bottom_mlp[i-1], self.bottom_mlp[i]))
        self.net_bottom.append(PEPMLP(self.opt, 'bot2', self.bottom_mlp[-1], self.embedding_size))
        
        self.net_top = nn.ModuleList()
        self.net_top.append(PEPMLP(self.opt, 'top0', self.top_mlp_insize, self.top_mlp[0]))
        for i,o in enumerate(self.top_mlp[1:], 1):
            self.net_top.append(PEPMLP(self.opt, 'top'+str(i+1), self.top_mlp[i-1], self.top_mlp[i]))
        self.net_top.append(PEPMLP(self.opt, 'top3', self.top_mlp[-1], 1))
...
#                 layers_ffp8_143 = ['.4.embs','.7.embs','.18.embs','.22.embs','top_linear_0','top_linear_1','top_linear_2','bottom_linear']
    ...
    def forward(self, Xi, Xv):
        ...
        Bottom MLP part
        """        
        bottom_mlp_result = activation(self.net_bottom[0](Xv))
        for layer in self.net_bottom[1:]:
            bottom_mlp_result = activation(layer(bottom_mlp_result))
...
#         embedding_array = [emb(Xi[:,i,:]) for i, emb in enumerate(self.dlrm_embeddings)]
        ...
        elif self.is_shallow_dropout: # interaction_op=='dot'
        ...
        Top MLP part
        """
        top_mlp_result = activation(self.net_top[0](top_mlp_input))
        for layer in self.net_top[1:-1]:
            top_mlp_result = activation(layer(top_mlp_result))
        top_mlp_result = self.net_top[-1](top_mlp_result)
    ...
    def init_weights(self):
        ...
        for name, param in model.named_parameters():
            ...
            if ('weight' in name) and ('net_' not in name): # weight and bias in the same layer share the same glorot
    ...
    def calc_sparsity(self):
        ...
        non_zero_values = 0
        for i in range(3):
            non_zero_values += torch.nonzero(self.net_bottom[i].sparse_v, as_tuple=False).size(0)
        for i in range(4):
            non_zero_values += torch.nonzero(self.net_top[i].sparse_v, as_tuple=False).size(0)
    ...
    def save_pruned_embedding(self, param, step_idx):
        ...
        if max_candidate_p == 0:
            ...
            if param <= max_candidate_p:
                ...
                mlp_save_path = self.opt['mlp_save_path'].format(num_parameter=param, layer='{layer}')
                mlp_save_dir, _ = os.path.split(mlp_save_path)
                if not os.path.exists(mlp_save_dir):
                    os.makedirs(mlp_save_dir)
                
                # np.save(emb_save_path, embedding)
                for i in range(3):
                    linear = self.net_bottom[i].sparse_v.detach().cpu().numpy()
                    np.save(mlp_save_path.format(layer='bot'+str(i)), linear)
                for i in range(4):
                    linear = self.net_top[i].sparse_v.detach().cpu().numpy()
                    np.save(mlp_save_path.format(layer='top'+str(i)), linear)
                    
                max_idx = self.candidate_p.index(max(self.candidate_p))
                self.candidate_p[max_idx] = 0
                print("*" * 80)
                print("Reach the target parameter: {}, save MLP with size: {}".format(max_candidate_p, param))
                print("*" * 80)
            elif step_idx == 0:
                # embedding = self._factorizer.model.get_embedding()
                mlp_save_path = self.opt['mlp_save_path'].format(num_parameter='initial_mlp', emb='{emb}')
                mlp_save_dir, _ = os.path.split(mlp_save_path)
                if not os.path.exists(mlp_save_dir):
                    os.makedirs(mlp_save_dir)
    ...
    def fit(self, Xi_train, Xv_train, y_train, Xi_valid=None, Xv_valid=None, 
            y_valid = None, early_stopping=True, refit=False, save_path = None, 
        ...
        for epoch in range(self.n_epochs):
            ...
            if epoch == self.warm and self.warm != 0:
                ...
                # optimizer = torch.optim.Adam(self.parameters(), lr=self.learning_rate*0.6, weight_decay=self.weight_decay)
...
#         print('Test dataset size',x_size)
        ...
        tn, fp, fn, tp = sklearn.metrics.confusion_matrix(y, np.round(y_pred)).ravel()
        TPR = tp/(tp+fn)
        TNR = tn/(tn+fp)
        return total_loss/x_size, total_metric, TPR, TNR, total_acc, inf_time
