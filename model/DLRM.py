...
class DLRM(torch.nn.Module):
    ...
    def __init__(self,field_size, feature_sizes, embedding_size = 64, is_shallow_dropout = True, dropout_shallow = [0.0,0.0],
                 ...
                 optimizer_type = 'sgd', is_batch_norm = False, verbose = False, random_seed = 0, weight_decay = 0.0,
                 loss_type = 'logloss',use_cuda = True, n_class = 1, greater_is_better = True, sparse = 0.9, warm = 10, 
                 numerical=13, top_mlp='512-512-256' , bottom_mlp='512-256',field_weight = False,full_interactions = False,is_bias = True):
        ...
        if self.is_batch_norm:
            setattr(self, 'net_' + 'bottom_batch_norm_' + str(0), nn.BatchNorm1d(self.deep_layers[0], momentum=0.005))
        ...
        if self.is_batch_norm:
            setattr(self, 'net_' + 'top_batch_norm_' + str(0), nn.BatchNorm1d(self.deep_layers[0], momentum=0.005))
        if self.is_deep_dropout:
    ...
    def forward(self, Xi, Xv):
        ...
        #26*2048*64
        if self.is_shallow_dropout: #True, 但是 Dropout(0)
...
#         print('Zflat',Zflat.shape)#[2048, 351]
#         print('bottom_mlp_result', bottom_mlp_result.shape)#[2048, 64]
        ...
        for i in range(Z.shape[1]):
            for j in range(i):
                interaction_result_list.append(Z[:,i,j].unsqueeze(0)) #[i][j]
        interaction_result = torch.cat(interaction_result_list)
        #print('interaction_result',interaction_result.shape)
        # 729*2048  +   2048*64   ->   2048*793
        top_mlp_input = torch.cat((interaction_result.transpose(0,1),bottom_mlp_result),1)
    ...
    def fit(self, Xi_train, Xv_train, y_train, Xi_valid=None, Xv_valid=None, 
            ...
            prune=0, prune_emb=0, prune_r=0, prune_deep=0, emb_r=1., emb_corr=1.,
...
if __name__ == "__main__":
    model = DLRM(field_size=39, feature_sizes=[20]*26)
    for name, param in model.named_parameters():
        print('name: ', name)
        # print('param: ', param)
        print("=======================")