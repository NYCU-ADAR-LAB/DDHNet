...
def read_data(file_path, emb_file, num_list, feature_dim_start=0, dim=39 ,sparse_dim_start = 13):
    ...
    for num, item in enumerate(cate_dict):
        if (num + 1 not in num_list)and num+1 > sparse_dim_start :
    ...
    for line in f:
        ...
        #indexs = [float(item) for i, item in enumerate(datas) if i not in num_list and i != 0]
        #values = [float(item) for i, item in enumerate(datas) if i in num_list]
