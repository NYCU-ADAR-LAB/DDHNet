...
class DLRM(torch.nn.Module):
    ...
    def __init__(self, field_size, feature_sizes, embedding_size = 64, is_shallow_dropout = True, dropout_shallow = [0.0,0.0],
                 ...
                 retrain=False, retrain_emb_sparsity=0, re_init=False):
        ...
        self.candidate_p = [int(sum(self.feature_sizes)*self.embedding_size * factor) for factor in [0.2, 0.125, 0.1, 0.05, 0.04, 0.03125, 0.03, 0.02, 0.01]]
        ...
        self.opt = {
            ...
            'emb_save_path': emb_save_path + '/{overall_sparsity}/{embid}.npy',
            'retrain_emb_sparsity': retrain_emb_sparsity,
            're_init': re_init
        }
...
#
    ...
    def calc_sparsity(self):
        ...
        params_l=[]
        total_params=0
        for i in range(len(self.dlrm_embeddings)):
            non_zero_values = torch.nonzero(self.dlrm_embeddings[i].sparse_v, as_tuple=False).size(0)
            base = torch.numel(self.dlrm_embeddings[i].sparse_v)
        ...
        percentage = 1 - (sum(params_l) / (sum(self.feature_sizes) * self.embedding_size))
    ...
    def save_pruned_embedding(self, sparsity_l, params_l, percentage, total_params):
        ...
        if max_candidate_p == 0:
            ...
            if total_params <= max_candidate_p:
                emb_save_path = self.opt['emb_save_path'].format(overall_sparsity=percentage, embid='{embid}') #'./tmp/embedding/{task}/{overall_sparsity}/{embid}.npy'
                ...
                emb_dims = np.round(self.embedding_size * (1 - sparsity_l))
                with open(emb_save_dir.replace('embedding', 'runs') + '/sparsity_{}_total_params_{:.4f}.txt'.format(percentage, total_params), 'w') as f:
    ...
    def fit(self, Xi_train, Xv_train, y_train, Xi_valid=None, Xv_valid=None, 
        ...
        for epoch in range(self.n_epochs):
            ...
            for i in range(batch_iter+1):
                ...
                if step_idx == 0:
                    emb_save_path = self.opt['emb_save_path'].format(overall_sparsity='initial_embedding', embid='{embid}')
                ...
                sparsity_l, params_l, overall_sparsity, total_params = self.calc_sparsity()
                if not self.retrain:
                    self.save_pruned_embedding(sparsity_l, params_l, overall_sparsity, total_params)
                self._writer.add_scalar('train/step_wise/mf_loss', loss.item(), step_idx)
                self._writer.add_scalar('train/step_wise/emb_sparsity', overall_sparsity, step_idx)
                if i % 2000 == 0:
                    print('[Epoch {}|Step {}|Flag {}|Sparsity {:.4f}|Params {}]'.format(epoch+1, step_idx+1, flag, overall_sparsity, total_params))
            ...
            sparsity_l, params_l, overall_sparsity, total_params = self.calc_sparsity()
            self._writer.add_scalar('train/epoch_wise/emb_sparsity', overall_sparsity, epoch)
            self._writer.add_scalar('train/epoch_wise/params', total_params, epoch)
            ...
            if is_valid:
                ...
                print("*" * 80)
                print('Embedding Sparsity %.2f%%, Parameters %d' % (overall_sparsity, sum(params_l)))
                print('Training [%d] Logloss: %.6f|AUC: %.6f|Acc: %.6f|Overall Sparsity %.2f%%|time: %.1f s' %
                  (epoch + 1, train_loss, train_eval,train_acc, 100 - (num_dnn+sum(params_l)) * 100. / num_total, time()-epoch_begin_time))
            
                print('Validation [%d] Logloss: %.6f|AUC: %.6f|Acc: %.6f|Overall Sparsity %.2f%%|time: %.1f s' %
                      (epoch + 1, valid_loss, valid_eval, valid_acc, 100 - (num_dnn+sum(params_l)) * 100. / num_total, time()-epoch_begin_time))
                if Best_model : 
    ...
    def post_eval(self,Xi_valid, Xv_valid, y_valid,load_path=None ):
        ...
        _, _, _, total_params = self.calc_sparsity()
        print('Model Parameters %d, Sparsity %.2f%%' % ((no_non_sparse+total_params), 100 - (no_non_sparse+total_params) * 100. / num_total))
        print('*' * 50)
        epoch_begin_time = time()
        valid_result = []
        valid_loss, valid_eval, valid_acc = self.eval_by_batch(Xi_valid, Xv_valid, y_valid, x_valid_size)
        ...
        print('Validation Logloss: %.6f|AUC: %.6f|Acc: %.6f|Overall Sparsity %.2f%%|time: %.1f s' %
                  (valid_loss, valid_eval, valid_acc, 100 - (no_non_sparse+total_params) * 100. / num_total, time()-epoch_begin_time))
        print('*' * 50)
